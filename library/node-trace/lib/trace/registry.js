...
class Registry {
	...
	getMetricsAsArray() {
//console.log("hello16",Object.keys(this._metrics).map(this.getSingleMetric, this));
	...
	metrics(opts) {
     ...
     // console.log("Hello3",this.getMetricsAsArray()
//                        .map(metric => this.getMetricAsPrometheusString(metric, opts))
  //                      .join('\n'));
	...
	registerMetric(metricFn) {
       ...
       // console.log("Hello17",metricFn.name);
		...
		this._metrics[metricFn.name] = metricFn;
               //  console.log("Hello18",this._metrics);
